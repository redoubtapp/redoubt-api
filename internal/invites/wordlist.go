...
// Each word is 4-5 letters for easier typing.
...
// GenerateInviteCode generates a random 4-word invite code.
func GenerateInviteCode() (string, error) {
	words := make([]string, 4)
	listSize := big.NewInt(int64(len(effShortWordlist)))

	for i := 0; i < 4; i++ {
		idx, err := rand.Int(rand.Reader, listSize)
		if err != nil {
			return "", err
		}
